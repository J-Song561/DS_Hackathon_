...
// =========================================================
// 4. (중요!) 페이지가 처음 로드될 때 스스로 실행하는 코드
// =========================================================
// 이 코드가 없으면 새로고침(F5) 했을 때 배너가 안 뜹니다.

if (location.href.includes("youtube.com/watch") || location.href.includes("youtube.com/shorts")) {
    console.log("CS: 페이지 로드 감지. 스스로 분석 시작.");
    
    // 가짜 데이터로 즉시 배너 생성
    const initialMockAnalysis = {
        riskLevel: '높음',
        summaryMessage: "이 영상은 '충격' 등 자극적인 단어를 3회 사용했습니다. 내용이 과장되었을 수 있으니 주의하세요.",
        issues: [
            { timestamp: '제목', text: '🔥 충격! 절대 클릭하지 마세요', reason: '자극적인 제목' },
    ...
    // 1초 뒤에 배너 띄우기 (유튜브 UI 로딩 대기)
    setTimeout(() => {
        injectWarningBanner(initialMockAnalysis);
    }, 1000);
}